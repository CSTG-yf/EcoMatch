...
- `train.jsonl`、`dev.jsonl`、`test.jsonl`：199 条官方题（当前 2.0.3 正式评估库，唯一正式评分依据）；
...
`evaluation/bank_nl2sql/official/2.0.3/`。2.0.3 是**唯一当前正式评分依据**，
2.0.2 作为不可变父版本继续保留。

2.0.3 包含：

- `bank-nl2sql-ground-truth-v2.0.2.xlsx`：从父版本逐字节继承的 ground-truth 工作簿（199 题）；
- `official-manifest.json`：`datasetVersion=2.0.3`、`canonicalReady=true`、
...
- `answer-amendment-audit-summary.json`：父版本答案修正的审计摘要；
...
2.0.3 不修改工作簿答案或数据库事实；它为 32 条此前无法由结果表完整证明的 train/dev
答案增加类型化 `expected.answerFacts`，必要时使用版本化 gold 查询返回省均值、构成占比、
人均值、区间均值、年初基线及收入结构。Fact v3 发布门禁为 train+dev 159/159 READY，
test.jsonl 与 2.0.2 逐字节一致。
...
evaluation\.venv\Scripts\python.exe evaluation/bank_nl2sql/amend_official_answer_facts.py `
  --parent evaluation/bank_nl2sql/official/2.0.2 `
  --spec evaluation/bank_nl2sql/answer-facts/2.0.3.json `
  --output evaluation/bank_nl2sql/official/2.0.3 `
  --update-current
```
...
`db/releases/2.0.3/` 是基于当前 2.0.3 官方工作簿冻结的**伴随导入包**，**不是运行时
...
从 v2.0.3 唯一事实源重建该包时，必须由生成器同时写入 SQLite、H2 脚本和
`database-manifest.json`，不得手工修改其中任一产物：

```powershell
evaluation\.venv\Scripts\python.exe evaluation/bank_nl2sql/db/build_database.py `
  evaluation/bank_nl2sql/official/2.0.3/bank-nl2sql-ground-truth-v2.0.2.xlsx `
  --sqlite-output evaluation/bank_nl2sql/db/releases/2.0.3/bank.sqlite `
  --h2-script-output evaluation/bank_nl2sql/db/releases/2.0.3/bank-h2.sql `
  --database-manifest-output evaluation/bank_nl2sql/db/releases/2.0.3/database-manifest.json `
  --official-version 2.0.3 `
  --source-relative-path evaluation/bank_nl2sql/official/2.0.3/bank-nl2sql-ground-truth-v2.0.2.xlsx
```
...
正式版必须使用 `official/CURRENT.json` 指向的2.0.3工作簿与
...
evaluation\.venv\Scripts\python.exe evaluation/bank_nl2sql/build_dataset.py `
  ...
  --official-manifest evaluation/bank_nl2sql/official/2.0.3/official-manifest.json `
  --output .local-dev/bank-nl2sql/rebuild-2.0.3

evaluation\.venv\Scripts\python.exe evaluation/bank_nl2sql/validate_dataset.py `
  .local-dev/bank-nl2sql/rebuild-2.0.3
```

正式版基于 v2.0.3 唯一事实源 SHA-256
...
`manifest.json` 记录 v2.0.3 的 answer-fact 溯源与 `templateOverlap` 风险披露，
绝不改变题目归属。
...
evaluation\.venv\Scripts\python.exe evaluation/bank_nl2sql/build_gold.py `
  evaluation/bank_nl2sql evaluation/bank_nl2sql/db/releases/2.0.3/bank.sqlite
...
`official_runtime_evaluation_v3.json` 固定：v2.0.3 数据库包、Fact v3 评分、独立会话、
...
1. 停止 standalone，使用上文 `Import-OfficialBankData.ps1` 导入 v2.0.3；再启动服务。
2. 创建目标环境的语义模型与聊天模型。
