...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.mode == "test" and (
            not args.acknowledge_final_test or args.run_registry is None
        ):
            raise OfficialRuntimeRunError(
                "test mode requires --acknowledge-final-test and --run-registry"
            )
        ...
        if args.mode == "test":
            ...
            if isinstance(existing_run, dict) and isinstance(
                existing_run.get("finalTestRunNumber"), int
            ):
