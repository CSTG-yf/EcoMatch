...
/**
 * Fixed, cacheable two-stage contract prompts for the model-owned bank NL2SQL route.
 *
 * <p>
 * The model makes the natural-language decision twice: it first states the requirements it
 * understood ({@code REQUIREMENTS} stage, {@link BankRequestContract}), then produces the
 * executable semantic plan ({@code PLAN} stage, {@link BankQueryPlan}). Deterministic code only
 * validates JSON shape, exact identifiers and the relationship between those two model artifacts.
 *
 * <p>
 * The system prompt is split into three stable parts so each stage only carries its own contract:
 * <ul>
 * <li>{@link #COMMON_FACT_PREFIX} - role constraints plus the full shared fact catalog generated
 * by {@link BankSemanticRegistry} (metrics, derived metrics, organizations, shared enums).</li>
 * <li>{@link #REQUIREMENTS_SYSTEM_PREFIX} - {@link BankRequestContract} format, natural-language
 * slot/intent judgement, CLARIFY conditions and REQUIREMENTS repair rules.</li>
 * <li>{@link #PLAN_SYSTEM_PREFIX} - {@link BankQueryPlan} format, requirements-to-plan mapping,
 * compiler-supported calculation/filter/aggregation/ranking/output contracts and PLAN repair
 * rules.</li>
 * </ul>
 * {@link #FIXED_SYSTEM_PREFIX} remains as the concatenation of all three parts for callers and
 * tests that need the full legacy prompt; it is not used by the stage-aware runtime path.
 */
public final class BankPlanPromptComposer {

    /**
     * Role constraints shared by both stages. Any change must bump the stage prefix versions and
     * re-warm the prefix cache.
     */
     ...
     */
    public static final String COMMON_FACT_PREFIX =
            (ROLE_PREFIX + "\n\n{{SHARED_CATALOG}}")
                    .replace("{{SHARED_CATALOG}}", BankSemanticRegistry.sharedCatalog()).strip();

    /** REQUIREMENTS-only section: contract format, slot judgement, CLARIFY, repair rules. */
    private static final String REQUIREMENTS_STAGE_SECTION = """
            ════════════════════════════════
            ...
            ════════════════════════════════
            {{FILTER_CONTRACT}}
            """.replace("{{FILTER_CONTRACT}}",
                    BankSemanticRegistry.filterContract()).strip();

    /** PLAN-only section: BankQueryPlan format, requirements mapping, compiler contracts, repair. */
    private static final String PLAN_STAGE_SECTION = """
            ...
            把 requirements_contract 视为只读事实：不重新识别或修改指标、机构、日期和意图；
            ...
            1. 指标只能是目录中完全一致的大写 ZB###；机构只能是完全一致的 ORG###。禁止近义词、拼音、
               小写代码、别名、物理字段名或自造代码。
            2. metrics 必须逐项包含 requirements_contract 声明的全部指标；不得默默补充或删除指标。
               不得增加目录中未被用户要求的指标。
            3. 日期只能写 YYYY-MM-DD；比较基期必须用 baselineStartDate 和 baselineEndDate 明确表达。
               对 comparison 非 NONE 且非 MOM_AND_YOY 的比较，当前期只能写在 startDate/endDate，
               基期只能写在 baselineStartDate/baselineEndDate，且 baselineEndDate 必须早于 startDate。
               ...
               比较基期当作普通当前值查询。
            4. “全省均值”只能使用目录允许的 benchmark/COMPARE/PROVINCE_AVERAGE 合同，不能写 SQL 或自行估算。
            5. 收到 <repair> 时，只根据 error、previous_candidate、requirements_contract 和目录修正；
               只改正 error 指出的非法槽位，并保留 previous_candidate 中其余已合法、仍满足
               requirements_contract 的槽位。仍只输出当前阶段要求的一份完整 BankQueryPlan JSON，
               不输出补丁或解释。
            """.replace("{{PLAN_CAPABILITY_CATALOG}}",
                    BankSemanticRegistry.planCapabilityCatalog()).strip();
    ...
    /** System prompt for the PLAN stage: shared facts plus the plan contract rules. */
    public static final String PLAN_SYSTEM_PREFIX =
            COMMON_FACT_PREFIX + "\n\n" + PLAN_STAGE_SECTION;

    /**
     * Legacy combined prefix for callers and tests that need every stage rule in one prompt. The
     * stage-aware runtime path uses {@link #REQUIREMENTS_SYSTEM_PREFIX} / {@link #PLAN_SYSTEM_PREFIX}
     * instead so each stage only pays attention cost for its own contract.
     */
