...
CREATE TABLE IF NOT EXISTS `s2_metric_org_mapping` (
    ...
    `updated_by` VARCHAR(100) NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_metric_org_mapping` (`metric_id`, `organization_code`, `external_metric_code`),
...
CREATE TABLE IF NOT EXISTS `s2_audit_event` (
    ...
    `sanitized_question` LONGTEXT,
    `question_hash` VARCHAR(64),
    `metric_codes` LONGTEXT,
    `sql_type` VARCHAR(32),
    `sql_digest` VARCHAR(64),
    `policy_ids` LONGTEXT,
    `masking_summary` LONGTEXT,
    ...
    UNIQUE KEY `uk_audit_event_hash` (`event_hash`),
    UNIQUE KEY `uk_audit_event_trace_previous_hash` (`trace_id`, `previous_hash`),
    KEY `idx_audit_event_trace_time` (`trace_id`, `event_time`),
    KEY `idx_audit_event_user_time` (`user_name`, `event_time`),
    KEY `idx_audit_event_org_time` (`organization_id`, `event_time`),
    KEY `idx_audit_event_anomaly_scope` (`event_type`, `user_name`, `organization_id`, `event_time`),
    KEY `idx_audit_event_type_time` (`event_type`, `event_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `s2_audit_rule` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_audit_rule_code` (`rule_code`),
    KEY `idx_audit_rule_type_enabled` (`rule_type`, `enabled`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `s2_security_alert` (
    ...
    `updated_by` VARCHAR(128) NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_security_alert_id` (`alert_id`),
    UNIQUE KEY `uk_security_alert_fingerprint` (`fingerprint`),
    KEY `idx_security_alert_status_time` (`status`, `last_seen`),
    KEY `idx_security_alert_rule_status` (`rule_id`, `status`),
    KEY `idx_security_alert_severity_time` (`severity`, `last_seen`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `s2_alert_action` (
    ...
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_alert_action_id` (`action_id`),
    KEY `idx_alert_action_alert_time` (`alert_id`, `created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
