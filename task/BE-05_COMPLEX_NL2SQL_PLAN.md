...
- 状态：已完成（2026-07-25）。端到端运行器、测试集访问门禁和本地最终运行登记已实现并通过测试。“银行问数”已绑定 `Text2SQL数据集`，且已核验该数据集含 21 个指标和 3 个维度。真实页面全量 dev 复测 `ui-dev-full-v11-score.json` 取得 36/36 解析、执行、结果一致，证明局域网模型已稳定生成可执行的受约束计划并经由实际 S2SQL、物理 SQL 与结果链路返回；采集器始终只允许 train/dev，冻结 test 保持显式最终验收门禁。
- Mode：`BDD_TDD`
...
- 状态：已完成（2026-07-25）。`BankQueryPlanCompiler` 只从已验证的语义 Schema 解析标识符，直接聚合、趋势、TopN、阈值和多指标查询生成 `QueryStructReq`；单指标变化与显式双指标比率生成受控 S2SQL 模板，并支持按任意已验证的语义维度分组。分组变化会对当前期与基期的完整维度键等值连接；比率使用两阶段 CTE 先计算分子与分母、再计算百分比，避免语义翻译器在嵌套函数中遗漏指标展开。机构、时间、过滤、输出列顺序和零分母都由代码固定处理，Schema 展示名会归一为业务标识；未知机构语义维度、未选中排序字段、无法表示的过滤条件或计算都会明确拒绝。新增查询指纹，并修复 `QueryStructReq` 将分组转为 `HashSet` 而破坏列顺序的问题。本轮补齐了受控单点/排名的银行结果契约：执行后将已返回的语义宽表确定性投影为 `org_code, org_name, metric_code, metric_value[, rank_position]`；比率则投影为 `org_code, org_name, numerator_value, denominator_value, ratio_percent`，两者均不改写 SQL 或数值。新增多指标变动的固定 CTE/`UNION ALL` 模板、显式跨期（如“上半年末到年末”）时间基线，以及 `MULTI_METRIC_CHANGE` 结果合同；页面验收样本 `TRAIN-H-34` 的解析、执行、结果一致率均为 100%（1/1）。在隔离 H2 数据库及 SQLite 基准库中，train/dev 的 151 条物理金标 SQL 均可成功执行；最新全量 dev 页面评测将运行中 H2 的 36 条可见结果逐列逐行与 SQLite 金标比较，19 个 SQL 特征全部为 100%，完成双方言结果等价验证。
- Mode：`BDD_TDD`
...
- 状态：已完成（2026-07-25）。候选排序、去重、单次纠错、错误封装和安全诊断已接入 `LLMSqlParser → Translator → Executor`；解析响应保留路线、指纹、输出契约和候选摘要，但不记录原始模型输出。完整 `headless/chat` Maven 回归为 100 通过、1 跳过；其中 `LLMRequestServiceTest` 明确验证银行开关关闭时保留 `ONE_PASS_SELF_CONSISTENCY`，非银行数据集在开关开启时也不会切入银行策略。已登录真实页面的完整 dev 36 条评测则覆盖自然语言到结果的受控路径。受控策略默认仍为单候选，现有服务行为不因候选机制而改变。

- 运行时错误终态（2026-07-24）：真实界面复测确认，无效银行查询计划在结构化修复后曾被吞掉并落入通用 Rule SQL 解析。现已在受约束路径把该错误传递为终态，阻止规则回退、`MapModeEnum.ALL` 二次解析和错误信息模型重写；前端只显示安全的中文引导语。无效比较请求约 11 秒返回失败终态，普通银行查询仍可成功执行。新增 `BankNl2SqlErrorTest`，并通过 25 项相关单测及聊天/语义服务跨模块编译。
- Mode：`BDD_TDD`
- Dependency：T5。
...
- 页面验收补充（2026-07-24，进行中）：新增 `run_ui_chat_capture.mjs` 和 `evaluate_ui_capture.py`。前者只在已登录的浏览器页面中通过 `#chatInput` 提交问题、等待页面表格终态并合并每页 10 行分页；后者按既有金标列、行、顺序和数值容差评分。API 运行器继续用于解析/编译/执行诊断，但不得单独作为产品页面效果结论。页面采集运行器只允许 train/dev，可按已知开发题 ID 进行单题验证；冻结 test 不可调用。
- 状态：已完成（2026-07-25）。在已登录的真实“银行问数”页面中，采集器以每题新建浏览器页签的方式提交完整 dev 36 条题目，只从页面渲染表格读取结果；最新独立复测 `.local-dev/bank-nl2sql/ui-dev-full-v11-score.json` 记录解析 100%（36/36）、执行 100%（36/36）、结果一致率 100%（36/36），复杂/普通/简单三组也均为 100%，且错误类别均为 `NONE`。本轮在同一真实页面复验新增的复杂训练样本 `TRAIN-H-34`，报告 `.local-dev/bank-nl2sql/ui-train-h34-score-v5.json` 记录解析、执行、结果一致率均为 100%（1/1）；已验证跨期多指标变动的数值、机构展示名和标准指标码。上述结果满足第 2.3 节的 dev 门禁；旧报告 `dev-full-candidate-ranker-v6.json`（66.67%）和 `dev-full-comprehensive-v4.json`（72.22%）保留为历史基线。由于 49 条冻结 test 已有一次 `REJECT` 验收并不能再用于迭代或证明新版提升，下一次正式盲测必须使用新的外部保留集。
...
- 状态：BLOCKED（外部保留集缺失，2026-07-25）。当前 DATA-02 冻结校验通过：官方金标 SQL 200/200 通过，源工作簿与全部数据产物哈希已写入 `release_manifest.json`。开发实现已冻结于 commit `23e94b8`，但现有 49 条内部 test 已登记 `REJECT`，不可再次作为新版的验收或调优依据；收到独立外部保留集后，须重新冻结 commit、模型配置和数据库哈希，再运行一次页面端到端盲测。

- 最新复验（2026-07-24）：Agent 33 在完整 49 条保留集的第 3 次登记运行中，解析与执行成功率均为 100%，结果一致率为 55.10%（27/49），低于第 1 次有效运行的 61.22%；第 2 次登记运行仅因评测进程未携带认证而得到 0/49，已标注为环境失败，不参与比较。复杂/普通/简单一致率为 33.33%/53.57%/88.89%。结论为 `REJECT`：不得将该版本标记为 BE-05 验收通过，且不得基于保留题做定向改动。
