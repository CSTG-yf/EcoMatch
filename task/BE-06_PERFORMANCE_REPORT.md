...
BE-06 已通过本地 H2 标准测试环境的安全、限流、结果集限制、超时取消和性能基线验证。2026-07-30 完成 5 分钟 QA-03 本地验收：100 次延迟采样平均 `5.26 ms`、P95 `7.38 ms`、P99 `9.59 ms`，稳定性阶段执行 295,541 次查询且失败数为 0，超时取消后数据库端残留语句为 0。
...
| --- | ---: | --- |
| 延迟采样数 | 100 | 通过 |
| 平均响应时间 | 5.26 ms | 通过，低于 3 秒 |
| P50 / P95 / P99 | 5.15 / 7.38 / 9.59 ms | 记录 |
| 稳定性时长 | 300 秒 | 通过 |
| 并发数 | 4 | 通过 |
| 稳定性成功 / 失败 | 295,541 / 0 | 通过 |
| JDBC 取消耗时 | 1,007 ms | 通过 |
| 取消后残留语句 | 0 | 通过 |
| 最终活动查询 / 可用许可 | 0 / 4 | 通过，资源已释放 |
| JVM 最大堆内存 | 742,946,464 bytes | 记录 |
| JVM 最大线程数 | 20 | 记录 |

机器可读报告为 `task/BE-06_QA03_H2_ACCEPTANCE_REPORT.json`。报告不包含 JDBC URL、账号、密码或 SQL。

同日对生产发行包执行首次启动和重复启动验证。验证过程中发现并修复 H2 文件库未自动初始化及 `s2_agent.chat_model_config` 容量不足问题；修复后空库和已有库均可持续启动，`/v3/api-docs` 返回 HTTP 200，网关统计接口可达并正确执行登录鉴权。

## QA-03 待验收项

- 在比赛目标数据库及数据规模上复测平均响应时间、P95 和 P99。
- 覆盖缓存命中与未命中、简单查询、复杂查询和跨库查询。
- 在目标数据库执行长时间并发稳定性测试并同步记录数据库服务端资源占用。
