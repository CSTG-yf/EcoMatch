...
- 统一采集解析、模型、翻译、执行和解释五个阶段的调用次数、累计耗时、平均耗时和最大耗时。
- 提供仅超级管理员可访问的 `GET /api/semantic/query/gateway/stats` 监控接口，同时返回网关运行快照和五阶段耗时。
...
- `QueryPerformanceMonitorTest`：校验五阶段耗时聚合、平均值和最大值。
...
- 最新实测结果：平均 `8.64 ms`、P95 `17 ms`、P99 `23 ms`，并发查询无拒绝。
