...
- 危险函数同时执行 AST 级校验，覆盖带引号或 schema 限定的函数名，以及投影、WHERE、HAVING、QUALIFY、JOIN、GROUP BY、ORDER BY 和表函数位置，避免文本变体绕过。
...
- 14 个关联 Maven 模块在 JDK 21 下完成编译，88 项安全、权限、性能和解释定向测试通过。
...
- 最新实测结果：平均 `9.59 ms`、P95 `16 ms`、P99 `22 ms`，并发查询无拒绝。
