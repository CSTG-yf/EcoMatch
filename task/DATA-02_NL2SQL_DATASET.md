...
- 生成 200 条官方正样本，字段包含问题、标准化意图、S2SQL、物理 SQL、预期结果摘要及结果哈希。
- 覆盖单表、多表、嵌套、聚合、同比、环比、TopN 和跨机构查询。
- 另生成 12 条增强样本，仅用于开发和鲁棒性测试，不进入官方准确率。
- 官方来源划分保持 120/40/40；无模板泄漏评测划分为训练 115、验证 36、测试 49，调整记录可由 manifest 逐题追溯。

## 自动验收

- `BankNl2SqlDatasetValidationTest` 冻结校验 200 条官方题、115/36/49 评测划分、ID 唯一性、`EXECUTE` 动作、SQL/S2SQL 审计模板一致性及全部金标 SQL 可解析为只读 `SELECT`。
